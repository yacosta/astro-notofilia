...
test('Philippines Victory Series catalog page lists both notes in Spanish', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Serie Victory No. 66' })).toBeVisible();
  await expect(page.locator('h1')).toHaveCount(1);
  await expect(page.locator('[data-catalog-mode="primary"]')).toHaveCount(0);
...
test('Philippines Victory Series catalog page lists both notes in English', async ({ page }) => {
  ...
  await expect(page.locator('[data-catalog-mode="primary"]')).toHaveCount(0);
  await expect(
    page.locator('main#main-content a[href="/en/collection/philippines/1-peso-victory-series-66/"]'),
  ).toBeVisible();
  await expect(
...
test('5 pesos Victory Series ficha shows the cropped stacked scan', async ({ page }) => {
  ...
  const img = page.locator(
    'main#main-content img[src="/uploads/philippines-treasury-certificate-5-pesos-victory-series-66-ce93f0dc.jpg"]:not([data-zoom-image])',
  );
...
test('homepage Logros del Mes features the Giori Lincoln Memorial uniface first', async ({ page }) => {
  ...
  await expect(first).toHaveAttribute(
    'href',
    '/coleccion/giori-press-test-note-lincoln-memorial-uniface/',
  );
  await expect(first).toContainText('Giori — Memorial Lincoln, uniface, RGMB1/0NSU');
  await expect(first.getByRole('img')).toHaveAttribute(
    'src',
    '/uploads/us-giori-press-test-note-lincoln-memorial-uniface-card.jpg',
  );
});

test('English homepage Logros features the Giori Lincoln Memorial uniface first', async ({ page }) => {
  await page.goto('/en/');
  const section = page.locator('section[aria-labelledby="logros-heading"]');
  ...
  await expect(first).toHaveAttribute(
    'href',
    '/en/collection/giori-press-test-note-lincoln-memorial-uniface/',
  );
  await expect(first).toContainText('Giori — Lincoln Memorial uniface, RGMB1/0NSU');
});

test('homepage Logros still features the Débora Arango P-458b face proof', async ({ page }) => {
  await page.goto('/');
  const section = page.locator('section[aria-labelledby="logros-heading"]');
  await expect(
    section.locator('a[href="/coleccion/colombia/banco-de-la-republica-2000-pesos-debora-arango-prueba-anverso/"]'),
  ).toBeVisible();
});
...
test('body copy on catalog, landings, and editorial pages uses the new page gutter', async ({
  ...
  const samples = [
    ...
    { path: '/coleccion/giori-press-test-note-lincoln-memorial-uniface/', selector: '#main-content' },
    { path: '/en/collection/giori-press-test-note-lincoln-memorial-uniface/', selector: '#main-content' },
...
test('mobile drawer lists virtual notaphily hubs and coin pages', async ({ page }) => {
  ...
  await expect(drawer.getByRole('link', { name: 'Reserva Federal', exact: true })).toHaveCount(0);
  ...
  await expect(drawer.getByRole('link', { name: 'Reserva Federal', exact: true })).toBeVisible();
  await expect(drawer.getByRole('link', { name: 'Certificados de Pago Militar', exact: true })).toBeVisible();
  await expect(drawer.getByRole('link', { name: 'Cupones de alimentos USDA', exact: true })).toBeVisible();
  await expect(drawer.getByRole('link', { name: 'Emisiones promocionales', exact: true })).toBeVisible();
