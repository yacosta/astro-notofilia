 ...
 * Series 66 1 peso photo (stacked obverse/reverse in sleeves).
 * Compress/format only — no crop, composite, or substitute.
 * See .cursor/skills/catalog-submitted-images/SKILL.md
 *
 * Usage:
 *   SOURCE=/path/to/original.png node scripts/process-filipinas-victory-image.mjs
 */
...
const SOURCE_CANDIDATES = process.env.SOURCE
  ? [process.env.SOURCE]
  : [
      path.join(ROOT, 'VICTORY SERIES 66 PHILIPPINES $1 VICTORY NOTE.png'),
      path.join(ROOT, 'public/uploads/VICTORY SERIES 66 PHILIPPINES $1 VICTORY NOTE.png'),
      path.join(ROOT, 'public/uploads/philippines-treasury-certificate-1-peso-victory-series-66-source.png'),
    ];
...
if (!SOURCE) {
  console.error(
    [
      'Missing user-submitted Philippines Victory Series 66 photo.',
      'Checked:',
      ...SOURCE_CANDIDATES.map((p) => `  - ${p}`),
      '',
      'Commit the original full-frame photo (both faces, sleeves included).',
...
const SLUG = `philippines-treasury-certificate-1-peso-victory-series-66-${HASH}`;
