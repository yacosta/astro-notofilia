/**
 * Encode catalog hero + responsive variants for the Banco de Rio Hacha
 * 1883 PMG proof photo (side-by-side front/back holders).
 *
 ...
 *
 * Usage:
 *   SOURCE=/path/to/original.png node scripts/process-rio-hacha-slab-image.mjs
 */
...
const SLUG = 'colombia-banco-de-rio-hacha-5-pesos-1883';
...
if (!SOURCE) {
  console.error(
    [
      'Missing user-submitted Banco de Rio Hacha slab photo.',
      'Checked:',
      ...
      'Then re-run: node scripts/process-rio-hacha-slab-image.mjs',
    ].join('\n'),
  );
  process.exit(1);
}
...
const jsonPath = path.join(ROOT, 'src/content/catalog/colombia--banco-de-rio-hacha-5-pesos-1883.json');
